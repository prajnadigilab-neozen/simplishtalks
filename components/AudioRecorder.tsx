...
interface AudioRecorderProps {
  onRecordingComplete: (blob: Blob, durationSeconds?: number, transcribedText?: string) => void;
  lessonId?: string;
  hideHistory?: boolean;
}
...
const AudioRecorder: React.FC<AudioRecorderProps> = ({ onRecordingComplete, lessonId, hideHistory }) => {
  ...
  const startRecording = async () => {
    try {
      ...
      // Reset speech recognition transcription
      transcribedTextRef.current = '';
      const SpeechRecognition = (window as any).SpeechRecognition || (window as any).webkitSpeechRecognition;
      if (SpeechRecognition) {
        const recognition = new SpeechRecognition();
        recognition.lang = 'en-IN'; // Indian English pronunciation
        recognition.interimResults = false;
        recognition.maxAlternatives = 1;
        recognition.onresult = (event: any) => {
          if (event.results && event.results[0] && event.results[0][0]) {
            transcribedTextRef.current = event.results[0][0].transcript;
            console.log("Speech recognition transcribed:", transcribedTextRef.current);
          }
        };
      ...
      mediaRecorder.onstop = async () => {
        ...
        // Wait a brief moment to let speech recognition finish processing
        setTimeout(() => {
          onRecordingComplete(audioBlob, recordingSeconds, transcribedTextRef.current);
        }, 400);
      };
