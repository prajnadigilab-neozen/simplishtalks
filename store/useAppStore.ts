...
import { CourseLevel, UserProgress, Module, UserRole, LevelStatus, PackageType, SnehiScenario, CourseFeedback } from '../types';
...
export const useAppStore = create<AppState>((set, get) => ({
    ...
    initialize: async (forceRefresh?: boolean) => {
        ...
        try {
            ...
            if (userId) {
                get().fetchMyFeedback().catch(e => console.error("Error loading feedback:", e));
            }
            set({ loading: false });
    ...
    fetchMyFeedback: async () => {
         ...
         if (!userId) return;
         try {
             const { getMyFeedback } = await import('../services/feedbackService');
             const fb = await getMyFeedback(userId);
             set({ feedback: fb });
         } catch (e) {
