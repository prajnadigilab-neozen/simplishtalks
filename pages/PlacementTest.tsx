...
const PlacementTest: React.FC = () => {
  ...
  const handleSkipEvaluation = async () => {
    ...
    setLoading(true);
    try {
      await setPlacementResult({
        suggestedLevel: CourseLevel.BASIC,
        score: 0,
        reasoning: "User skipped the placement test. Assigned to default Basic level.",
        reasoningKn: "ಬಳಕೆದಾರರು ಪ್ರವೇಶ ಪರೀಕ್ಷೆಯನ್ನು ಬಿಟ್ಟಿದ್ದಾರೆ. ಆದ್ದರಿಂದ ಆರಂಭಿಕ ಹಂತವನ್ನು ನೀಡಲಾಗಿದೆ."
      });
      navigate('/packages');
    } catch (e) {
      console.error(e);
      ...
      <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        {step === 1 && (
          <div className="space-y-6">
            ...
            <div className="flex gap-4">
              ...
              <button onClick={handleSkipEvaluation} className="px-8 py-5 border-2 border-slate-200 dark:border-slate-700 text-blue-600 dark:text-blue-400 rounded-3xl font-black uppercase tracking-widest hover:bg-slate-50 dark:hover:bg-slate-800 transition-all">Skip (ನಂತರ )</button>
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <div className="space-y-6">
            ...
            <button onClick={handleSkipEvaluation} className="w-full py-5 border-2 border-slate-200 dark:border-slate-700 text-blue-600 dark:text-blue-400 rounded-3xl font-black uppercase tracking-widest hover:bg-slate-50 dark:hover:bg-slate-800 transition-all">Skip (ನಂತರ )</button>
          </div>
        )}
        ...
        {step === 4 && (
          <div className="space-y-6">
            ...
            <div className="flex flex-col gap-3">
              ...
              <button onClick={handleSkipEvaluation} className="w-full py-5 border-2 border-slate-200 dark:border-slate-700 text-blue-600 dark:text-blue-400 rounded-3xl font-black uppercase tracking-widest hover:bg-slate-50 dark:hover:bg-slate-800 transition-all">
                Skip (ನಂತರ )
              </button>
            </div>
          </div>
