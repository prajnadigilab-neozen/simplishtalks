...
const LessonEditor: React.FC = () => {
            ...
            if (lessonId) {
                ...
                if (lesson) {
                    const scenario = lesson.scenario ? {
                        ...lesson.scenario,
                        characterStr: lesson.scenario.character ? (typeof lesson.scenario.character === 'object' ? `${lesson.scenario.character.en || ''} | ${lesson.scenario.character.kn || ''}` : String(lesson.scenario.character)) : '',
                        objectiveStr: lesson.scenario.objective ? (typeof lesson.scenario.objective === 'object' ? `${lesson.scenario.objective.en || ''} | ${lesson.scenario.objective.kn || ''}` : String(lesson.scenario.objective)) : '',
                    } : undefined;
                    ...
                    setEditingLesson({
                        ...
                        english_text_to_read: lesson.englishTextToRead,
                        transcription_to_read_kannada_phonetic: lesson.transcriptionToReadKannadaPhonetic,
                        transcription_to_read_transliteration: lesson.transcriptionToReadTransliteration,
                        scenario,
    ...
    const parseLessonJson = (json: any, currentLesson: any) => {
        ...
        if (json.content && json.module_metadata) {
            ...
            const choiceBased = agent?.situational_conversational_tree || agent?.choice_based_roleplay || null;

            let initialMessage = agent?.initial_greeting || "Hello! Let's practice our English.";
            let character = splitBilingual(agent?.name || 'AI Coach');

            if (choiceBased) {
                const firstTurn = choiceBased.find((t: any) => t.turn === 1);
                if (firstTurn && firstTurn.ai_prompt) {
                    if (!agent?.initial_greeting) {
                        initialMessage = firstTurn.ai_prompt.english || initialMessage;
                    }
                    if (!agent?.name && firstTurn.ai_prompt.speaker) {
                        character = splitBilingual(firstTurn.ai_prompt.speaker);
                    }
                }
            }
            ...
            const scenario: any = {
                character,
                characterStr: `${character.en} | ${character.kn}`,
            ...
            if (choiceBased) {
                scenario.choice_based_roleplay = choiceBased;
            }
            ...
            // Parse Speak Content
            const speak = content.step_3_speak;
            let speakTextContent = currentLesson.speak_text_content;
            let englishTextToRead = currentLesson.english_text_to_read || null;
            let transcriptionToReadKannadaPhonetic = currentLesson.transcription_to_read_kannada_phonetic || null;
            let transcriptionToReadTransliteration = currentLesson.transcription_to_read_transliteration || null;

            if (speak) {
                if (typeof speak === 'object') {
                    speakTextContent = JSON.stringify(speak);
                    englishTextToRead = speak.english_text_to_read || speak.english || speak.transcription_to_read || null;
                    transcriptionToReadKannadaPhonetic = speak.transcription_to_read_kannada_phonetic || speak.transcription_to_read_kannada || speak.kannada_phonetic || speak.kannada || null;
                    transcriptionToReadTransliteration = speak.transcription_to_read_transliteration || speak.transliteration || null;
                } else {
                    speakTextContent = String(speak);
                }
            }
            ...
            return {
                ...
                speak_text_content: speakTextContent,
                english_text_to_read: englishTextToRead,
                transcription_to_read_kannada_phonetic: transcriptionToReadKannadaPhonetic,
                transcription_to_read_transliteration: transcriptionToReadTransliteration,
                scenario
            };
        }
