...
export async function updateProfile(id: string, updates: any): Promise<{ success: boolean; error?: string }> {
  try {
    ...
    if (typeof sanitizedUpdates.personal_address === 'string') {
        sanitizedUpdates.personal_address = DOMPurify.sanitize(sanitizedUpdates.personal_address);
    }
    if (typeof sanitizedUpdates.employment_status === 'string') {
        sanitizedUpdates.employment_status = DOMPurify.sanitize(sanitizedUpdates.employment_status);
    }
    if (typeof sanitizedUpdates.pincode === 'string') {
        sanitizedUpdates.pincode = DOMPurify.sanitize(sanitizedUpdates.pincode);
    }
...
export async function getAllUsers(): Promise<any[]> {
  try {
    const { data, error } = await supabase
      .from('profiles')
      .select('id, full_name, phone, place, role, created_at, avatar_url, is_restricted, package_type, package_status, topup_amount, snehi_access_enabled')
    ...
    const mapped = (data || []).map(u => ({
      ...u,
      role: mapRole(u.role),
      snehi_access_enabled: u.snehi_access_enabled || false,
  ...
  try {
    ...
    if (!error && profile) {
      cachedProfile = {
        ...
        snehiAccessEnabled: profile.snehi_access_enabled || false,
        isLoggedIn: true
      };
    } else {
      if (!error && !profile) {
