    ...
    for (const item of queue) {
        try {
            ...
            if (item.action === 'UPSERT_PROGRESS') {
                ...
                let { error } = await supabase.from('lessons').upsert(payload, { onConflict: 'id' });
                
                if (error && (error.code === 'PGRST204' || (error.message && (error.message.includes('column') || error.message.includes('english_text_to_read'))))) {
                    console.warn("⚠️ Remote schema cache does not have new speak columns. Stripping and falling back to speak_text_content JSON serialization...");
                    
                    const fallbackObj: any = {};
                    if (payload.english_text_to_read) fallbackObj.english_text_to_read = payload.english_text_to_read;
                    if (payload.transcription_to_read_kannada_phonetic) fallbackObj.transcription_to_read_kannada_phonetic = payload.transcription_to_read_kannada_phonetic;
                    if (payload.transcription_to_read_transliteration) fallbackObj.transcription_to_read_transliteration = payload.transcription_to_read_transliteration;
                    
                    if (Object.keys(fallbackObj).length > 0) {
                        fallbackObj.instruction = "Read the English sentences aloud. If you feel hesitant, read the Kannada script right below it—it spells out the exact English pronunciation to give you instant confidence!";
                        payload.speak_text_content = JSON.stringify(fallbackObj);
                    }
                    
                    delete payload.english_text_to_read;
                    delete payload.transcription_to_read_kannada_phonetic;
                    delete payload.transcription_to_read_transliteration;
                    
                    const retryResult = await supabase.from('lessons').upsert(payload, { onConflict: 'id' });
